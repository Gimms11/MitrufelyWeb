...
- **Backend (`FastAPI`):**
  ...
  - **Validaciones estrictas de Negocio (Esquemas Pydantic V2):**
    - _Categoría:_ Nombre único, obligatorio, entre 2 y 100 caracteres.
    - _Producto:_ Nombre único, obligatorio, entre 2 y 150 caracteres. Precio obligatorio, estrictamente positivo (`CHECK precio > 0`). FK `id_categoria` debe ser un ID existente en la tabla `categorias`.
