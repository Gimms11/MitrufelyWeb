 ...
 *
 * FIX: toda la tarjeta es ahora clickeable (wrapping Link), no solo el botón "Ver más".
 */
...
export function TrufaCard({ producto }: TrufaCardProps) {
  ...
  return (
    ...
    >
      {/* Toda la tarjeta envuelta en Link para navegar al detalle */}
      <Link
        to={`/producto/${producto.slug}`}
        id={`home-trufa-card-${producto.id_producto}`}
      ...
      >
        ...
        <div className="px-2 pb-2 flex-grow flex flex-col justify-between">
          ...
          <div className="flex flex-col mb-4 gap-1.5">
            <h4
              className="font-black text-[#2a1115] text-lg leading-tight group-hover:text-[#5c0f1b] transition-colors"
              style={{ fontFamily: "'Outfit', sans-serif" }}
            >
              {normalizeName(nombre)}
            </h4>
            <p className="text-xl font-black text-[#5c0f1b]">
          ...
          <span
            className={`w-full inline-flex items-center justify-center font-black rounded-full py-2.5 text-sm transition-all active:scale-95 ${
              isAvailable
                ? 'bg-[#5c0f1b] text-white group-hover:bg-[#7a1525]'
                : 'bg-stone-100 text-stone-400'
            }`}
          >
            {isAvailable ? 'Ver más' : 'Agotado'}
          </span>
        </div>
      </Link>
    </motion.div>
